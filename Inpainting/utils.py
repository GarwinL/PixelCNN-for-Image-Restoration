...
# Masked noise
def add_noise(img, sigma, mean, mask):   
    ...
    img_noisy = torch.tensor(img + mask*gauss);
    
    return img_noisy

def add_inpainting(img, rate):
    nr_pixels = np.prod(img.size())
    n = int(np.round(nr_pixels*rate)) # Number of pixels to remove
    
    # Shuffle through index
    idx = np.arange(nr_pixels)
    np.random.shuffle(idx)
    
    # Choose idx and convert to 2D
    idx_2d = np.unravel_index(idx[0:n],img.size())
    
    # Create mask and corrupt image
    img_corrupt = torch.tensor(img)
    mask = torch.zeros(img.size())
    
    img_corrupt[idx_2d] = 0.0
    mask[idx_2d] = 1.0
