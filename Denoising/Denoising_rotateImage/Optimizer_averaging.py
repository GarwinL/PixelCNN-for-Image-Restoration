...
from Denoising.Denoising_rotateImage.denoising_averaging import Denoising
from Denoising.utils import PSNR, img_rot90, c_ssim, add_noise
from Denoising.Denoising_rotateImage.config import BaseConfig
import time
import keyboard
...
#    x = torch.tensor(y.clamp(min=-1,max=1).to(device));
    ...
    description = 'rotateAvg_80x80_denoising_' + prior_loss + '_alpha=' + str(config.alpha)
...
#        denoise = Denoising(optimizer, scheduler, image, y, net, sigma, alpha, net_interval=1, writer_tensorboard=writer_tensorboard)
    ...
    fig.savefig(config.directory.joinpath("Plot.png"))
