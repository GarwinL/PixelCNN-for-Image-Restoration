...
def denoise_parameter(par, config, net):
    ...
    image_list = torch.zeros((len(test_loader), config.n_epochs, 1, 1, config.crop_size, config.crop_size))
    ...
    for cnt, (image, label) in enumerate(test_loader):
        ...
        if config.linesearch:
            ...
            optimizer = config.optimizer(params, lr=config.lr) #, momentum=0.88) 
