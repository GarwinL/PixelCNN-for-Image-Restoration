...
import torch
from PixelCNNpp.utils import log_logistic_mixture, log_prob_from_logits, log_logistic_mixture_continous, discretized_mix_logistic_loss
...
#     #logistic = discretized_mix_logistic_loss(x, logit)
#     #print(torch.sum(logistic))
#     
#     return logistic
# 
# =============================================================================
...
def logposterior(x, y, sigma, alpha, logit):
    #x = x.reshape(y.shape)
    return torch.sum(nllh(x,y,sigma)) + torch.sum(alpha*nll_prior(logit,x));