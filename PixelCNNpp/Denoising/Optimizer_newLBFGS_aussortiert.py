...
#    x = torch.tensor(y.clamp(min=-1,max=1).to(device));
    ...
    optimizer = LBFGS(params, lr=0.01, history_size=10, line_search='Wolfe', debug=True) 
    ...
    start = time.time()
    for i in range(2000):#config.n_epochs):
        #with torch.no_grad():       
...
#        denoise = Denoising(optimizer, scheduler, image, y, net, sigma, alpha, net_interval=1, writer_tensorboard=writer_tensorboard)
    ...
    gradient = gradient.view((80,80)).detach().cpu().numpy()
