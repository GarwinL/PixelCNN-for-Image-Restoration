...
class nin(nn.Module):
    ...
    def forward(self, x):
        ...
        # [batch_size, x, y, feature]
        x = x.permute(0, 2, 3, 1)
        # out = self.lin_a(x) # Faster option?: Should work -> test
...
class down_shifted_deconv2d_interpol(nn.Module):
    def __init__(self, num_filters_in, num_filters_out, filter_size=(2,3), stride=(1,1)):
        super(down_shifted_deconv2d_interpol, self).__init__()
        
        self.interpol = torch.nn.Upsample(scale_factor=2)
        
        self.conv = down_shifted_conv2d(num_filters_in, num_filters_out, filter_size)
...
class down_right_shifted_deconv2d_interpol(nn.Module):
    def __init__(self, num_filters_in, num_filters_out, filter_size=(2,3), stride=(1,1)):
        super(down_right_shifted_deconv2d_interpol, self).__init__()
        
        self.interpol = torch.nn.Upsample(scale_factor=2)
        
        self.conv = down_right_shifted_conv2d(num_filters_in, num_filters_out, filter_size)
...
class gated_resnet(nn.Module):
    def __init__(self, num_filters, conv_op, nonlinearity=concat_elu, skip_connection=0):
        ...
        self.conv_input = conv_op(2 * num_filters, num_filters) # 2*num_filters cuz of concat elu
        
        if skip_connection != 0 : 
            self.nin_skip = nin(2 * skip_connection * num_filters, num_filters)

        self.dropout = nn.Dropout2d(0.5)
        self.conv_out = conv_op(2 * num_filters, 2 * num_filters) # 2*num_filters cuz of concat elu


    def forward(self, og_x, a=None):
        # p=og_x(dim=1) -> x: 2p because of nonlinearity
        ...
        x = self.dropout(x)
        x = self.conv_out(x)
        # split 2p in 2 times of p and combine it by nonlinearity (product and sigmoid)
