...
### Log Logistic Mixture Continous - derivative of Sigmoid
def nl_logistic_mixture_continous(x, l):
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    # Pytorch ordering
    x = x.permute(0, 2, 3, 1)
    l = l.permute(0, 2, 3, 1)
...
def img_rot(img):
    rand_nr = np.random.randint(1,5)
    
    if rand_nr == 1: # no rotation
        img_rot = img
    elif rand_nr == 2: # 90 degrees
        inv_idx = torch.arange(img.size(-1)-1, -1, -1).long()    
        img_rot = img.permute(0,1,3,2)[..., inv_idx]   
    elif rand_nr == 3: # 180 degrees
        inv_idx = torch.arange(img.size(-1)-1, -1, -1).long()  
        inv_idy = torch.arange(img.size(-2)-1, -1, -1).long()  
        img_rot = img[:, :, :, inv_idx]  
        img_rot = img_rot[:, :, inv_idy, :]
    elif rand_nr == 4: # 270 degrees
        inv_idy = torch.arange(img.size(-2)-1, -1, -1).long()    
        img_rot = img.permute(0,1,3,2)[:, :, inv_idy, :]
