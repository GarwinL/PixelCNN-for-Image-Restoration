...
from PixelCNNpp.layers_with_interpolation import gated_resnet, down_shifted_conv2d, down_right_shifted_conv2d, nin, down_shifted_deconv2d_interpol, down_right_shifted_deconv2d_interpol
from PixelCNNpp.utils import concat_elu
...
class PixelCNNLayer_up(nn.Module):
    ...
    def forward(self, u, ul):
        ...
        for i in range(self.nr_resnet):
            u  = self.u_stream[i](u)
            # a = skip connection, add to input
...
class PixelCNN(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10, 
        ...
        self.upsize_u_stream  = nn.ModuleList([down_shifted_deconv2d_interpol(nr_filters, nr_filters, 
                                                    stride=(2,2)) for _ in range(2)])
        
        self.upsize_ul_stream = nn.ModuleList([down_right_shifted_deconv2d_interpol(nr_filters, 
                                                    nr_filters, stride=(2,2)) for _ in range(2)])
